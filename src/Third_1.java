...
class NilDenominatorException extends Exception {
...
class Vector {
    ...
    public double getSin(Vector other) throws NilDenominatorException {
        double len_mul = this.getLen() * other.getLen();
        if (len_mul == 0)
            throw new NilDenominatorException("Произведение длинн равно нулю! Деление на ноль");
        return this.getScalar(other) / (len_mul);
    }

    public double getVectMul(Vector other) throws NilDenominatorException {
        double sinus = this.getSin(other);
        if (sinus == 0)
            throw new NilDenominatorException("Синус равен нулю! Деление на ноль");
    ...
    public boolean isCross(Vector other) throws NilDenominatorException {
